---
description: Request rule element. Une ou plusieurs règles sont facultatives dans l’élément <ruleset>.
seo-description: Request rule element. Une ou plusieurs règles sont facultatives dans l’élément <ruleset>.
seo-title: règle
...
Request rule element. Une ou plusieurs règles sont facultatives dans l’ `<ruleset>` élément.
...
`RequestType` = *&quot;`types`&quot;*: Facultatif. Indique le contexte d’entrée auquel la règle s’applique. *`types`* est un  séparé par des virgules, qui peut inclure un ou plusieurs jetons répertoriés dans le tableau suivant. Si `RequestType` elle n’est pas spécifiée, la règle s’applique aux demandes reçues sur tous les  pris en charge.
...
<table id="table_4935E1ED03624DA6AF3F8DC9AAA10237"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>Application aux demandes de contenu statique </p> </td> 
  </tr> 
 </tbody> 
</table>

**`Name = "text"`**: Facultatif. Utilisé pour identifier l’ `<rule>` élément dans les journaux de débogage et les messages d’erreur.

`  *`Attribut`* ="value"`: Facultatif. `<rule>` peuvent définir l’un des attributs suivants dans n’importe quelle combinaison. Si elle est spécifiée et que la règle est correctement mise en correspondance, elle remplace les attributs de catalogue correspondants pour cette requête. Default is `RequestType="is"`.
...
Les attributs Expiration remplacent uniquement les valeurs d’attribut par défaut. Le remplacement est ignoré si une `catalog::Expiration` valeur spécifique s’applique à la requête.
...
Si `<substitution>` n’est pas spécifié, aucun chemin d’accès ou  de transformation de ne se produit, mais tous les attributs de catalogue spécifiés sont remplacés. Si `<substitution>` est vide, la sous-chaîne correspondante est supprimée.

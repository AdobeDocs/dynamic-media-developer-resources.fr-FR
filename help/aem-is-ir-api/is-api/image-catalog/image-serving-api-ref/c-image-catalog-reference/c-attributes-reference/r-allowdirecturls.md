...
Active ou désactive la prise en charge des URL absolues incorporées dans `src=` et les `mask=` commandes. Lorsque cette option est désactivée, seules les URL relatives à `attribute::RootUrl` sont autorisées.
...
Héritée de `default::AllowDirectUrls` si non définie ou si vide.

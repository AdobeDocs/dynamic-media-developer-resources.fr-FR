...
Chaîne de texte. Si spécifié, doit être une valeur `catalog::Id` valide dans ce catalogue d’images ou un chemin d’accès relatif (à `attribute::RootPath`) ou absolu à un fichier image accessible par le serveur d’images.

...
Utilisation de l’URI de données pour l’attribut `src` et de l’URL du serveur d’images pour l’attribut `data-src` :
...
Plusieurs commandes de diffusion d’images ou noms de paramètres d’image prédéfinis sont séparés par le caractère &quot;`&`&quot;. Si la valeur d’une commande de diffusion d’images contient une virgule, cette virgule est remplacée par `%2C`. Les noms des paramètres d’image prédéfinis sont encadrés de signes dollar ( `$`).

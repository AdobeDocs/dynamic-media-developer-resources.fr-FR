...
A l’aide de cet ID et des règles spécifiées avec `attribute::LocaleMap` et `attribute::LocaleStrMap`, la diffusion d’images applique la traduction facultative des ID de catalogue et les  de chaînes.

## Propriétés {#section-1854a9902b884d9b8e8e713b6635723f}

Demande, commande. S’applique à l’intégralité de la requête, y compris les requêtes imbriquées/incorporées, quel que soit l’endroit où elle est spécifiée. `locId` doit inclure uniquement des caractères ASCII imprimables. Ignoré si aucun mappage de  n’est défini dans le catalogue principal de cette requête. Une erreur est renvoyée si la valeur vide ou non valide `locId` est spécifiée et qu’aucune règle par défaut n’est définie dans `attribute::DefaultLocale`.

...
Lorsqu’elle est spécifiée dans une image, un texte ou un calque de couleur uni, *`opacity`* applique l’intégralité du calque, y compris tous les calques d’effet associés, tandis que *`fillOpacity`* s’applique uniquement au contenu du calque principal. Lorsque spécifié dans un calque d’effet, *`opacity`* s’applique au calque d’effet, tandis que *`fillOpacity`* est ignorée.

L’opacité effective du contenu du calque principal est ( *`opacity`* &#42; *`fillOpacity`* / 100). L’opacité effective des calques d’effet est (principale) *`opacity`* &#42; effet *`opacity`* / 100).
...
L’opacité du texte dans cet exemple est 90&#42;70/100=63 % et l’opacité du calque d’effet est de 90&#42;50/100 = 45 %.

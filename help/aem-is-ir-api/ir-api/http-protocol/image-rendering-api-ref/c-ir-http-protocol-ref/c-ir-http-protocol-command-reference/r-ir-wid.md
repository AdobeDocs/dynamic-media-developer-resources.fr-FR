...
L’image n’est pas complétée si les deux `wid=` et `hei=` sont spécifiés et `wid`/ `hei` est différent des proportions de l’image.
...
Peut se produire n’importe où dans la requête. Le redimensionnement de l’image avec `wid=`, `hei=`ou `scl=` ne modifie pas la valeur de résolution d’impression incorporée dans l’image de réponse. Ignoré si `scl=` survient après `wid=` ou `hei=` dans la séquence de commandes.

## Par défaut {#section-c7c6efa03d864592a3398b6f1de5a0b0}

Si aucune `wid=`, `hei=`ou `scl=` n’est spécifiée, l’image de réponse est mise à l’échelle pour s’adapter à la taille définie par `attribute::DefaultPix`. Si `attribute::DefaultPix` est vide, l’image de réponse aura la même taille que l’image de  de la vignette.

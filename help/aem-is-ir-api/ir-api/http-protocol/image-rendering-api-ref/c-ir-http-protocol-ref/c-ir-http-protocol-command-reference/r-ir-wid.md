...
L’image n’est pas complétée si `wid=` et `hei=` sont spécifiés et `wid`/ `hei` diffère des proportions de l’image.
...
Peut se produire n’importe où dans la requête. Le redimensionnement de l’image avec `wid=`, `hei=` ou `scl=` ne modifie pas la valeur de résolution d’impression incorporée dans l’image de réponse. Ignoré si `scl=` survient après `wid=` ou `hei=` dans la séquence de commande.

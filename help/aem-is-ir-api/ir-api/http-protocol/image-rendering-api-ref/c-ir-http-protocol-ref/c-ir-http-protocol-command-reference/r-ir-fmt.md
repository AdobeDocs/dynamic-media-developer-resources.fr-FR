...
*`format`* par défaut, `attribute::Format`et par *`tiffCompression`* défaut `attribute::TiffEncoding`. *`pixelType`* par défaut, `rgb` si `icc=` n’est pas spécifié, sinon il correspond au type de pixel du profil ICC spécifié.

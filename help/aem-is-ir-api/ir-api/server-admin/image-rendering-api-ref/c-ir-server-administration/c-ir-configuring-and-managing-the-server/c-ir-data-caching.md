---
description: Le rendu d’image tire parti des fonctionnalités de mise en cache de la diffusion d’images pour enregistrer les données d’image de réponse, les données intermédiaires pour les requêtes imbriquées/intégrées et les données d’image source (telles que les textures) obtenues à partir d’une source externe.
solution: Experience Manager
title: Mise en cache des données
feature: Dynamic Media Classic,SDK/API
role: Developer,Admin,User
exl-id: c7be9254-0dd4-4ec3-8448-86a18b9bea0d
TQID: 'https://experienceleague.adobe.com/irRmL6olYd2pzmUyyrGHlg5E-v61jJwjEWtx-tCaFBk'
product_v2:
  - id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2:
  - id: a01bfd36-4ab8-4bf8-9dc0-5b45b890552e
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
topic_v2:
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: 2ff64206b7448a1a122696facd2669be68b6b9ff
workflow-type: tm+mt
source-wordcount: 78
ht-degree: 0%

---

# Mise en cache des données{#data-caching}

Le rendu d’image tire parti des fonctionnalités de mise en cache de la diffusion d’images pour enregistrer les données d’image de réponse, les données intermédiaires pour les requêtes imbriquées/intégrées et les données d’image source (telles que les textures) obtenues à partir d’une source externe.

Reportez-vous à la section Documentation du service d’images pour une description des mécanismes de mise en cache.

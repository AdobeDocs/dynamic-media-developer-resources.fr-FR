...
* Le nombre de lignes ne dépasse pas le nombre maximum de `\copyfitmaxlines` et le nombre de lignes explicites dans la saisie de texte.
...
* La variable `id=` est actuellement requise lors de l’utilisation de la fonction `rect=` avec une demande d’image ou de masque.

## Différences connues textPs= par rapport à text= {#section-16ede4c13a7648feb0d2fc93341fd4aa}

* Les italiques synthétiques sont rendus moins inclinés que lors de l’utilisation de `text=`.
...
* Lorsque vous utilisez une société définie dans l’URL et une autre société définie pour la variable `src=` ou `mask=` modifier, vous devez préfixer une barre oblique vers l’entreprise définie pour `src=` ou `mask=` pour que cette forme de demande fonctionne.
...
* La recherche basée sur des fichiers n’est pas prise en charge lorsque la traduction des paramètres régionaux est activée en entreprise. [!DNL ini] fichier .
* Le serveur d’images n’écrit pas non fermé [!DNL Photoshop] chemins correctement.

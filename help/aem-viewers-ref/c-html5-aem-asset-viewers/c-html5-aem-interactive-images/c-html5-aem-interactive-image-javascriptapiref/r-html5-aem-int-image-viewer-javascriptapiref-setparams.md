...
Définit un ou plusieurs paramètres sur une valeur donnée. La syntaxe de l’argument de méthode est identique à une chaîne de requête d’URL. Autrement dit, il représente les paires nom=valeur séparées par `&`. Dans une chaîne de requête, les noms et les valeurs sont codés en pourcentage au moyen du codage UTF8. Avant d’appeler `init()`, ce paramètre doit être appelé.

...
description: Toute la personnalisation visuelle et la plupart des personnalisations de comportement pour la visionneuse d’images interactives s’effectuent en créant une feuille CSS personnalisée.
keywords: sensible
...
Toute la personnalisation visuelle et la plupart des personnalisations de comportement pour la visionneuse d’images interactives s’effectuent en créant une feuille CSS personnalisée.
...
Lors de la création d’une feuille CSS personnalisée, n’oubliez pas que la visionneuse attribue une `.s7interactiveimage` classe à son élément DOM conteneur. Si vous utilisez un fichier CSS externe transmis avec la commande, utilisez `style=` class comme classe parent dans le `.s7interactiveimage` sélecteur descendant de vos règles CSS. Si vous ajoutez des styles incorporés sur la page Web, qualifiez également ce sélecteur avec un ID de l’élément DOM conteneur comme suit :
...
Voici un exemple basé sur « sprite » d’ajout du style du même bouton de zoom avant précédemment :

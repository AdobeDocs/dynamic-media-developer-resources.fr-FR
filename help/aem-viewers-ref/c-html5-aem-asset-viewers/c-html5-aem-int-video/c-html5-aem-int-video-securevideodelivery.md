...
Selon la méthode de publication de [!DNL Dynamic Media] vidéo que vous utilisez dans Adobe Experience Manager, l’attribut de configuration `VideoPlayer.ssl` s’applique différemment, comme illustré ci-dessous :

* Si vous publiez une vidéo [!DNL Dynamic Media] avec une URL, vous ajoutez des `VideoPlayer.ssl` à l’URL. Par exemple, pour forcer la diffusion sécurisée d’une vidéo, vous ajoutez `&VideoPlayer.ssl=on` à la fin de l’exemple d’URL de visionneuse suivant :
...
* Si vous publiez une vidéo [!DNL Dynamic Media] avec du code intégré, vous ajoutez `VideoPlayer.ssl` à la liste des autres paramètres de configuration de la visionneuse dans le fragment de code intégré. Par exemple, pour forcer la diffusion vidéo en HTTPS, vous ajoutez des `&VideoPlayer.ssl=on` comme dans l’exemple suivant :

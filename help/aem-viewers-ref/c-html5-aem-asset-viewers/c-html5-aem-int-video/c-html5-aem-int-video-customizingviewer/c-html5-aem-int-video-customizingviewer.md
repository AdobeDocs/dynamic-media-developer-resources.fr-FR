...
Le deuxième groupe de marqueurs CSS contient `.s7mouseinput` et `.s7touchinput`. `.s7touchinput` est défini si l’appareil actuel dispose de fonctionnalités d’entrée tactile ; dans le cas contraire,  `.s7mouseinput` est utilisé. Ces marqueurs sont principalement destinés à créer des éléments d’entrée de l’interface utilisateur avec des tailles d’écran différentes pour différents types d’entrée, car la saisie tactile nécessite normalement des éléments plus volumineux.

Le troisième groupe de marqueurs CSS contient `.s7device_landscape` et `.s7device_portrait`. `.s7device_landscape` est défini si l’appareil tactile est en orientation paysage ;  `.s7device_portrait` est utilisé lorsque le périphérique tactile est pivoté vers l’orientation portrait. Ces marqueurs CSS sont destinés uniquement aux systèmes de bureau.
...
Dans l’exemple suivant, la barre de contrôle vidéo est positionnée à 138 pixels au-dessus du bas de la visionneuse si l’appareil tactile est en orientation portrait, puis déplacez-le tout en bas de la visionneuse dans tous les autres cas :

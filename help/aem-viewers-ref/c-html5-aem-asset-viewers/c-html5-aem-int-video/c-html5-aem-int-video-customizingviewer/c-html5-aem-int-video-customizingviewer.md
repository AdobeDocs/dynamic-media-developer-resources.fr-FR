...
description: Toutes les personnalisations visuelles et la plupart des personnalisations comportementales de la visionneuse de vidéo interactive sont effectuées en créant un fichier CSS personnalisé.
keywords: réactif
...
Toutes les personnalisations visuelles et la plupart des personnalisations comportementales de la visionneuse de vidéo interactive sont effectuées en créant un fichier CSS personnalisé.
...
La visionneuse est fournie avec deux fichiers CSS prêts à l’emploi, pour les modèles de couleurs « clair » et « sombre ». La version « sombre » est utilisée par défaut, mais il est facile de passer à la version « claire » en utilisant le CSS standard suivant :
...
Pour faciliter la création d’une feuille de style CSS en responsive design, la visionneuse prend en charge les marqueurs CSS. Ces marqueurs sont des classes CSS spéciales. Ils sont affectés dynamiquement à l’élément de conteneur de visionneuse de niveau supérieur en fonction de la taille de la visionneuse au moment de l’exécution et du type d’entrée utilisé sur l’appareil actuel.
...
Le deuxième groupe de marqueurs CSS contient `.s7mouseinput` et `.s7touchinput`. Le marqueur `.s7touchinput` est défini si l’appareil actuel dispose de fonctionnalités d’entrée tactile ; dans le cas contraire, `.s7mouseinput` est utilisé. Ces marqueurs sont principalement destinés à créer des éléments d’entrée de l’interface utilisateur avec différentes tailles d’écran pour différents types d’entrée, car normalement l’entrée tactile nécessite des éléments plus grands.

Le troisième groupe de marqueurs CSS contient `.s7device_landscape` et `.s7device_portrait`. Le marqueur `.s7device_landscape` est défini si l’appareil tactile est en orientation paysage ; `.s7device_portrait` est utilisé lorsque l’appareil tactile est tourné en orientation portrait. Ces marqueurs CSS sont destinés à être utilisés sur des systèmes de bureau uniquement.

L’exemple de CSS suivant définit la taille du bouton de lecture/pause sur 28 x 28 pixels sur les systèmes dotés d’une entrée de souris et 56 x 56 pixels sur les appareils tactiles. En outre, il masque complètement le bouton si la taille de la visionneuse est réduite de manière significative :
...
Dans l’exemple suivant, la barre de contrôle vidéo se trouve à 138 pixels au-dessus du bas de la visionneuse si l’appareil tactile est en orientation portrait. Dans tous les autres cas, il est déplacé en bas de la visionneuse :
...
Voici la documentation de référence sur les éléments de l’interface utilisateur qui s’applique à la visionneuse de vidéo interactive :

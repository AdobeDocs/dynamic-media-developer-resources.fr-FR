...
description: Toute la personnalisation visuelle et la plupart des personnalisations de comportement pour la visionneuse de vidéos interactives s’effectuent en créant un CSS personnalisé.
keywords: sensible
...
Toute la personnalisation visuelle et la plupart des personnalisations de comportement pour la visionneuse de vidéos interactives s’effectuent en créant un CSS personnalisé.
...
La visionneuse est fournie avec deux fichiers CSS prêts à l’emploi, pour les couleurs « claires » et « sombres ». La version « sombre » est utilisée par défaut, mais il est facile de passer à la version « claire » en utilisant le CSS standard suivant :
...
Lors de la création d’une feuille CSS personnalisée, n’oubliez pas que la visionneuse attribue une `.s7interactivevideoviewer` classe à son élément DOM conteneur. Si vous utilisez un fichier CSS externe transmis avec `style=` la commande, utilisez `.s7interactivevideoviewer` la classe comme classe parent dans le sélecteur de descendants de vos règles CSS. Si vous appliquez des styles incorporés sur la page Web, qualifiez ce sélecteur avec un ID de l’élément DOM conteneur comme suit :
...
Pour faciliter la création d’une feuille CSS adaptée, la visionneuse prend en charge les marqueurs CSS. Ces marqueurs sont des classes CSS spéciales. Ils sont affectés dynamiquement à l’élément conteneur de visionneuse de niveau supérieur en fonction de la taille de la visionneuse d’exécution et du type d’entrée utilisé sur le périphérique actuel.

Le premier groupe de marqueurs CSS comprend `.s7size_large`, `.s7size_medium`et `.s7size_small` les classes. Ils sont appliqués en fonction de la zone d’exécution du conteneur de la visionneuse. Si la zone de visionneuse est égale ou supérieure à la taille d’un moniteur de bureau commun, est `.s7size_large` utilisée ; si la zone est proche d’une tablette commune, elle `.s7size_medium` est affectée. Pour les zones similaires aux écrans de téléphone mobile, `.s7size_small` est définie. L’objectif principal de ces marqueurs CSS est de créer différentes dispositions d’interface utilisateur pour différents écrans et tailles de visionneuses.
...
Le troisième groupe de marqueurs CSS contient `.s7device_landscape` et `.s7device_portrait`. Le marqueur `.s7device_landscape` est défini si l’appareil tactile est en orientation paysage ; `.s7device_portrait` il est utilisé lorsque l’appareil tactile est pivoté en orientation portrait. Ces marqueurs CSS sont destinés à être utilisés uniquement sur les systèmes de bureau.
...
Dans l’exemple suivant, la barre de contrôle vidéo est positionnée à 138 pixels au-dessus du bas de la visionneuse si l’appareil tactile est en orientation portrait. Dans tous les autres cas, il est déplacé vers le bas de la visionneuse :
...
Voici la documentation de référence des éléments d’interface utilisateur qui s’applique à la visionneuse de vidéos interactive :

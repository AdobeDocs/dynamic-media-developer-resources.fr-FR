...
Lorsque vous créez un CSS personnalisé, n’oubliez pas que la visionneuse attribue `.s7smartcropvideoviewer` classe à son élément DOM conteneur. Si vous utilisez un fichier CSS externe transmis avec la commande `style=`, utilisez `.s7smartcropvideoviewer` classe comme classe parente dans le sélecteur descendant pour vos règles CSS. Si vous incorporez des styles à la page web, qualifiez également ce sélecteur avec un identifiant de l’élément DOM du conteneur comme suit :
...
Pour faciliter la création d’une feuille de style CSS responsive, la visionneuse prend en charge les marqueurs CSS qui sont affectés dynamiquement à des classes CSS spéciales à l’élément de conteneur de visionneuse de niveau supérieur. Cette affectation est basée sur la taille de la visionneuse au moment de l’exécution et le type d’entrée utilisé sur l’appareil actuel.
...
L’exemple de CSS suivant définit la taille du bouton de lecture/pause sur 28 x 28 pixels sur les systèmes dotés d’une entrée de souris et 56 x 56 pixels sur les appareils tactiles. En outre, le bouton est complètement masqué si la taille de la visionneuse devient réduite :
...
L’utilisation de marqueurs CSS est le moyen le plus flexible de créer un CSS réactif conçu. Cette flexibilité vous permet de cibler non seulement la taille de l’écran de l’appareil, mais également la taille réelle de la visionneuse, ce qui peut s’avérer utile pour les mises en page des pages en Responsive Design.
...
Voici la documentation de référence sur les éléments de l’interface utilisateur qui s’applique au recadrage intelligent de la visionneuse de vidéos :

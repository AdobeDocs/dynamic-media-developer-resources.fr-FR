...
L’effet d’icône prend en charge le sélecteur d’attributs `state`. `state="play"` est utilisé lorsque la vidéo est mise en pause au milieu de la lecture et `state="replay"` est utilisé lorsque la tête de lecture se trouve à la fin du flux.

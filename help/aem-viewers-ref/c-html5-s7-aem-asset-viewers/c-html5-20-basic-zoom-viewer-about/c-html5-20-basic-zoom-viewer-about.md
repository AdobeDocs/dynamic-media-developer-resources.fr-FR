...
Le mode contextuel est le plus courant pour les périphériques mobiles. La page Web charge la visionneuse à l’aide de l’appel `window.open()` JavaScript, d’un élément `A` HTML correctement configuré ou de toute autre méthode appropriée.
...
1. Ajouter le fichier JavaScript de la visionneuse sur votre page Web.
   ...
   Pour créer une visionneuse, vous devez ajouter une balise de script dans l’en-tête HTML. Avant d’utiliser l’API du lecteur de contenu, veillez à inclure [!DNL BasicZoomViewer.js]. Le [!DNL BasicZoomViewer.js] fichier se trouve sous le [!DNL html5/js/] sous-dossier de votre déploiement des visionneuses IS standard :
...
1. Définition de la taille de la visionneuse
   ...
   Vous pouvez définir la taille statique de la visionneuse en la déclarant pour la classe CSS de `.s7basiczoomviewer` niveau supérieur en unités absolues ou en utilisant `stagesize` des modificateurs.

   Vous pouvez placer le dimensionnement en CSS directement sur la page HTML ou dans un fichier CSS de visionneuse personnalisée, qui est ensuite affecté à un enregistrement de paramètre prédéfini de visionneuse dans SPS, ou transmis explicitement à l’aide d’une commande de style.
...
1. Création et initialisation de la visionneuse.
   ...
   Voici un exemple de création d’une instance de visionneuse, en transmettant au constructeur les options de configuration minimales nécessaires et en appelant la `init()` méthode. L’exemple suppose que `basicZoomViewer` il s’agit de l’instance de visionneuse ; `s7viewer` est le nom de l&#39;espace réservé `DIV`; `http://s7d1.scene7.com/is/image/` est l’URL de diffusion d’images et `Scene7SharedAssets/Backpack_B` est le fichier :

...
Le `{Object}` est une référence au composant SDK de visionneuse. La méthode renvoie `null` si la variable `componentId` n’est pas un composant de visionneuse pris en charge ou si le composant n’a pas encore été créé par la logique de visionneuse.

...
Exemple - Pour configurer l’en-tête de boîte de dialogue avec marge intérieure, icône de 24 x 17 pixels, titre en gras de 16 points et bouton de fermeture de 28 x 28 pixels. Enfin, positionnez-le à deux pixels du haut et à deux pixels de la droite du conteneur de boîte de dialogue :
...
Exemple - pour configurer un pied de boîte de dialogue avec un bouton Annuler de 64 x 34 et un bouton Envoyer un e-mail de 82 x 34, avec la couleur de texte et la couleur d’arrière-plan différentes pour chaque état de bouton :
...
>[!NOTE]
>
>Ce message prend en charge le sélecteur d’attributs `state` avec les valeurs possibles suivantes : `verifyerror`, `senderror` et `sendsuccess`. Le sélecteur d’attributs `verifyerror` est défini lorsqu’un message s’affiche en raison d’un échec de validation d’entrée en ligne ; `senderror` est défini lorsqu’un service de messagerie principal signale une erreur ; `sendsuccess` est défini lorsque l’envoi de l’e-mail réussit. Il est ainsi possible de mettre en forme le message différemment selon l’état de la boîte de dialogue.
...
Exemple - Pour configurer un message afin d’utiliser une police en gras de dix points. De plus, elle doit avoir une hauteur de ligne de 25 pixels, une marge intérieure de 20 pixels sur la gauche, utiliser une icône de point d’exclamation, du texte rouge en cas d’erreur et aucune icône et du texte vert en cas de succès :

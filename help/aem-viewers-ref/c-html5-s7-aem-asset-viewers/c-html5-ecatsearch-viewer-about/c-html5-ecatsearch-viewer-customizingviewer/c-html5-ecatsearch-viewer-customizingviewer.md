...
Lors de la création d’une page CSS personnalisée, gardez à l’esprit que la visionneuse affecte `.s7ecatalogsearchviewer` à son élément DOM de conteneur. Si vous utilisez un fichier CSS externe transmis avec la variable `style=` commande, utiliser `.s7ecatalogsearchviewer` classe en tant que classe parente dans le sélecteur descendant pour vos règles CSS. Si vous effectuez des styles intégrés sur la page web, vous devez également qualifier ce sélecteur avec un identifiant de l’élément DOM du conteneur comme suit :

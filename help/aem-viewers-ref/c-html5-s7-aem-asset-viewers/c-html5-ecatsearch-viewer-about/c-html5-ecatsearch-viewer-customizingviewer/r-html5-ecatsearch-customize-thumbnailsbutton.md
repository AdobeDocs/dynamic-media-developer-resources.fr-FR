...
>[!NOTE]
>
>Ce bouton prend en charge les sélecteurs d’attributs `state` et `selected`, qui peuvent être utilisés pour appliquer différents habillages à différents états de bouton. En particulier, `selected='true'` correspond à l’état de la visionneuse lorsque le mode de miniature est actif et `selected='false'` correspond à l’état par défaut avec la vue principale.

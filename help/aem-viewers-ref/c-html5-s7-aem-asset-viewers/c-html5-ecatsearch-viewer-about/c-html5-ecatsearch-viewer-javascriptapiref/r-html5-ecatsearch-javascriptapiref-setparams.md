...
Définit un ou plusieurs paramètres sur une valeur donnée. La syntaxe de l’argument de méthode est identique à une chaîne de d’URL. Autrement dit, il représente les paires nom=valeur séparées par [!DNL `&`]. Tout comme dans une chaîne de , les noms et les valeurs sont codés en pourcentage à l’aide de l’UTF8. Avant d’appeler [!DNL `init()`], ce paramètre doit être appelé.

...
[!DNL `[InfoPanelPopup.|<containerId>_infoPanelPopup.]template= *`modèle`*`]
...
En supposant que la réponse du serveur d’informations renvoie le nom du produit sous la forme variable [!DNL `$1$` et que l’URL de l’image du produit soit renvoyée sous la forme variable [!DNL `$2$`.

...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Fait défiler la liste des nuances dans la barre d’échantillons. </p> <p> Si l’image est à l’état de réinitialisation et que le <span class="codeph"> paramètre de transition de </span> cadre est défini sur diapositive, le fichier est modifié avec l’animation de la diapositive. Pour les autres modes de <span class="codeph"> transition de cadre </span> , le mouvement effectue le défilement de page natif. </p> <p> Si l’image est agrandie, elle déplace l’image horizontalement. Si l’image est déplacée vers le bord de la vue et qu’un glissement est effectué dans la même direction, le mouvement effectue le défilement natif de la page. </p> </td> 
  </tr> 
  <tr> 
...
Il est recommandé d’utiliser une page HTML prête à l’emploi pour le mode d’opération contextuel. La page HTML prête à l’emploi est appelée `ZoomViewer.html` et se trouve sous le `html5/` sous-dossier de votre déploiement des visionneuses IS standard, comme dans l’exemple suivant :
...
1. Ajouter le fichier JavaScript de la visionneuse sur votre page Web.
   ...
   Pour créer une visionneuse, vous devez ajouter une balise de script dans l’en-tête HTML. Avant d’utiliser l’API du lecteur de contenu, veillez à inclure [!DNL ZoomViewer.js]. Le [!DNL ZoomViewer.js] fichier se trouve sous le [!DNL html5/js/] sous-dossier de votre déploiement des visionneuses IS standard :
...
1. Définition de la taille de la visionneuse.
   ...
   Vous pouvez voir le comportement d’une visionneuse externe fixe dans l’exemple suivant. Notez que lorsque vous passez d’un jeu à l’autre, la taille de la visionneuse externe ne change pas :
   ...
   Pour rendre statiques les dimensions de la vue principale, définissez la taille de la visionneuse en unités absolues pour le composant `Container` SDK interne à l’aide du sélecteur `.s7zoomviewer` CSS ou à l’aide du modificateur `.s7container` `stagesize` CSS.
...
1. Création et initialisation de la visionneuse.
   ...
   Une fois les étapes ci-dessus terminées, vous créez une instance de `s7viewers.ZoomViewer` classe, vous transmettez toutes les informations de configuration à son constructeur et vous appelez `init()` la méthode sur une instance de lecteur.
   ...
   Voici un exemple de création d’une instance de visionneuse, de transmission des options de configuration minimales nécessaires au constructeur et d’appel de la `init()` méthode. Cet exemple suppose que `zoomViewer` il s’agit de l’instance de visionneuse, `s7viewer` du nom de l’espace réservé DIV, `http://s7d1.scene7.com/is/image/` de l’URL de diffusion d’images et `Scene7SharedAssets/ImageSet-Views-Sample` du fichier.
...
## Intégration de taille flexible avec définition de la largeur et de la hauteur {#section-3674e6c032594441a6576b7fb1de6e64}
...
## Incorporation à l’aide d’une API basée sur un paramètre {#section-44e014925f24418b900696003855c0a9}

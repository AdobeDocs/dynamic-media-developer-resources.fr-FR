...
La visionneuse de zoom en ligne représente un fichier JavaScript principal et un ensemble de fichiers d’aide (un seul code JavaScript est inclus avec tous les composants du SDK de la visionneuse utilisés par cette visionneuse, ressources et CSS) téléchargés par la visionneuse au moment de l’exécution.

La visionneuse de zoom en ligne peut être utilisée à la fois en mode contextuel à l’aide d’une page HTML prête à l’emploi fournie avec les visionneuses de diffusion d’images ou en mode incorporé où elle est intégrée dans une page Web de cible à l’aide d’une API documentée.
...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> Active la vue de la fenêtre déroulante ou les modifications entre le niveau de zoom principal et le niveau de zoom secondaire dans les échantillons, sélectionne une nouvelle miniature. </p> </td> 
  </tr> 
  <tr> 
...
Différentes pages Web ont des besoins différents en ce qui concerne le comportement des visiteurs. Il arrive qu’une page Web fournisse un lien cliquable qui ouvre le lecteur dans une fenêtre de navigateur distincte. Dans d’autres cas, il peut être nécessaire d’incorporer directement la visionneuse dans la page d’hébergement. Dans ce dernier cas, la page Web peut avoir une mise en page statique ou utiliser une conception adaptée qui s’affiche différemment sur différents périphériques ou pour différentes tailles de fenêtre de navigateur. Pour répondre à ces besoins, le lecteur prend en charge trois modes de fonctionnement principaux : fenêtre contextuelle, incorporation de taille fixe et incorporation réactive.

**Fenêtre contextuelle**

En mode contextuel, le lecteur s’ouvre dans une fenêtre ou un onglet distinct du navigateur Web. Elle prend toute la zone de la fenêtre du navigateur et s’ajuste lorsque la fenêtre du navigateur est redimensionnée ou que l’orientation du périphérique est modifiée.

Ce mode est le plus courant pour les périphériques mobiles. La page Web charge la visionneuse à l’aide d’un appel `window.open()` JavaScript, d’un élément `A` HTML correctement configuré ou de toute autre méthode appropriée.

Il est recommandé d’utiliser la page HTML prête à l’emploi pour le mode contextuel appelé `FlyoutViewer.html`. Il se trouve sous le [!DNL html5/] sous-dossier de votre déploiement standard de visionneuses d’images :
...
En mode incorporé, le lecteur est ajouté à la page Web existante, qui peut déjà contenir du contenu client qui n’est pas lié au lecteur. En règle générale, le lecteur n’occupe qu’une partie de l’espace réservé aux pages Web.

Les principales utilisations sont les pages Web orientées vers les ordinateurs de bureau ou les tablettes, ainsi que les pages Web réactives qui ajustent automatiquement la mise en page en fonction du type de périphérique.
...
Lors de l’utilisation du mode d’incorporation de conceptions réactives avec la visionneuse de zoom en ligne, veillez à spécifier des points d’arrêt explicites pour l’image de vue principale à l’aide du `imagereload` paramètre. Idéalement, faites correspondre vos points d’arrêt aux points d’arrêt de la largeur de la visionneuse, comme l’indique le CSS de la page Web.
...
1. Définition de la taille de la visionneuse.
   ...
   Voir [Personnalisation de la visionneuse](../../c-html5-s7-aem-asset-viewers/c-html5-inlinezoom-viewer-about/c-html5-inlinezoom-viewer-customizingviewer/c-html5-inlinezoom-viewer-customizingviewer.md#concept-82f8c71adbe54680a0c2f83f81e5f451) de zoom en ligne pour plus d’informations sur le style de la visionneuse avec CSS.
...
1. Création et initialisation de la visionneuse.
   ...
   Voici un exemple de création d’une instance de visionneuse, en transmettant les options de configuration minimales nécessaires au constructeur et en appelant la `init()` méthode. L’exemple suppose que `inlineZoomViewer` il s’agit de l’instance de visionneuse ; `s7viewer` est le nom de l&#39;espace réservé `DIV`; `http://s7d1.scene7.com/is/image/` est l’URL de diffusion d’images ; et `Scene7SharedAssets/ImageSet-Views-Sample` est l’actif :
...
* au lieu de définir une taille de visionneuse fixe à l’aide d’unités absolues, utilisez un fichier CSS qui définit la visionneuse `width` et `height` la valeur 100 % comme suit :

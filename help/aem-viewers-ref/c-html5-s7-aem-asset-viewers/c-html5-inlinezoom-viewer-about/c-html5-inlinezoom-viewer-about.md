---
description: La visionneuse de zoom en ligne est une visionneuse d’images. Il affiche une image statique avec la version agrandie affichée sur cette image statique lorsqu’un utilisateur survole ou touche le  principal du. Cette visionneuse fonctionne avec des visionneuses d’images et la navigation s’effectue à l’aide de nuances. Il est conçu pour fonctionner sur les ordinateurs de bureau et les périphériques mobiles.
keywords: responsive
seo-description: La visionneuse de zoom en ligne est une visionneuse d’images. Il affiche une image statique avec la version agrandie affichée sur cette image statique lorsqu’un utilisateur survole ou touche le  principal du. Cette visionneuse fonctionne avec des visionneuses d’images et la navigation s’effectue à l’aide de nuances. Il est conçu pour fonctionner sur les ordinateurs de bureau et les périphériques mobiles.
seo-title: Zoom intégré
...
La visionneuse de zoom en ligne représente un fichier JavaScript principal et un ensemble de fichiers d’aide (un seul code JavaScript inclut tous les composants du SDK de la visionneuse utilisés par cette visionneuse, ressources, CSS) téléchargés par la visionneuse au moment de l’exécution.

La visionneuse de zoom intégrée peut être utilisée en mode contextuel à l’aide d’une page HTML prête à l’emploi fournie avec les visionneuses de diffusion d’images ou en mode intégré, où elle est intégrée dans une page Web de  à l’aide d’une API documentée.
...
La visionneuse de zoom en ligne prend en charge les mouvements tactiles simples et multipoints courants dans d’autres applications mobiles.
...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> Active le  de la fenêtre déroulante ou modifie le niveau de zoom principal et secondaire dans les échantillons, sélectionne une nouvelle miniature. </p> </td> 
  </tr> 
  <tr> 
...
Le comportement des visiteurs varie selon les pages Web. Il arrive qu’une page Web fournisse un lien cliquable qui ouvre le lecteur dans une fenêtre de navigateur distincte. Dans d’autres cas, il peut être nécessaire d’incorporer directement le lecteur de contenu dans la page d’hébergement. Dans ce dernier cas, la page Web peut avoir une mise en page statique ou utiliser une conception adaptée qui s’affiche différemment sur différents périphériques ou pour différentes tailles de fenêtre de navigateur. Pour répondre à ces besoins, le lecteur prend en charge trois modes de fonctionnement principaux : contextuelle, incorporation de taille fixe et incorporation réactive.

**Fenêtre contextuelle**

En mode contextuel, le lecteur s’ouvre dans une fenêtre ou un onglet distinct du navigateur Web. Il prend toute la zone de la fenêtre du navigateur et s’ajuste lorsque la fenêtre du navigateur est redimensionnée ou que l’orientation du périphérique est modifiée.
...
Il est recommandé d’utiliser la page HTML prête à l’emploi pour le mode contextuel appelé `FlyoutViewer.html`. Il se trouve sous le [!DNL html5/] sous-dossier de votre déploiement standard des visionneuses d’hébergements d’images :

`<s7viewers_root>/html5/FlyoutViewer.html`

Il est également nécessaire que le composant FlyoutZoomView soit configuré pour fonctionner en mode de zoom intégré. Il est recommandé d’utiliser le `Scene7SharedAssets/Universal_HTML5_Zoom_Inline` paramètre prédéfini prêt à l’emploi pour la visionneuse de zoom en ligne ou un paramètre prédéfini personnalisé dérivé de celui-ci. La personnalisation visuelle peut être réalisée en appliquant une page CSS personnalisée.
...
Les principales utilisations sont les pages Web orientées vers les ordinateurs de bureau ou les tablettes, ainsi que les pages Web réactives qui ajustent automatiquement la disposition en fonction du type de périphérique.
...
Lors de l’utilisation du mode d’incorporation de conception réactive avec la visionneuse de zoom en ligne, veillez à spécifier des points d’arrêt explicites pour l’image  principale à l’aide du `imagereload` paramètre. Dans l’idéal, faites correspondre vos points d’arrêt aux points d’arrêt de la largeur de la visionneuse, comme le dicte le CSS de la page Web.
...
1. Définition de la taille de la visionneuse.
   ...
   Voir [Personnalisation de la visionneuse](../../c-html5-s7-aem-asset-viewers/c-html5-inlinezoom-viewer-about/c-html5-inlinezoom-viewer-customizingviewer/c-html5-inlinezoom-viewer-customizingviewer.md#concept-82f8c71adbe54680a0c2f83f81e5f451) de zoom intégrée pour plus d’informations sur le style de la visionneuse avec CSS.
...
1. Création et initialisation de la visionneuse.
   ...
   Voici un exemple de création d’une instance de visionneuse, de transmission des options de configuration minimales nécessaires au constructeur et d’appel de la `init()` méthode. L’exemple suppose `inlineZoomViewer` qu’il s’agit de l’instance de visionneuse ; `s7viewer` est le nom de l’espace réservé `DIV`; `http://s7d1.scene7.com/is/image/` est l’URL de diffusion d’images ; et `Scene7SharedAssets/ImageSet-Views-Sample` est l’actif :
   ...
   Le code suivant est un exemple complet d’une page Web triviale qui incorpore la visionneuse de zoom en ligne avec une taille fixe :
...
* au lieu de définir une taille de lecteur fixe à l’aide d’unités absolues, utilisez une feuille de style CSS qui définit la visionneuse `width` et `height` à 100 % comme suit :

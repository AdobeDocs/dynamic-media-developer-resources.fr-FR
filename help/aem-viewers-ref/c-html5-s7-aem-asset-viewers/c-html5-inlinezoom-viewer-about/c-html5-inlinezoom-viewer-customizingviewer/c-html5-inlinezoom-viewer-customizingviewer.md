...
seo-title: Personnalisation de la visionneuse de zoom intégrée
...
Il est également important de conserver la déclaration CSS suivante de la page CSS de visionneuse par défaut, car elle est nécessaire pour la fonctionnalité de zoom en ligne :
...
L’utilisation de marqueurs CSS est la méthode la plus souple de création de pages CSS adaptées qui vous permet de cible non seulement de la taille d’écran du périphérique, mais également de la taille réelle du lecteur, ce qui peut s’avérer utile pour les mises en page de pages Web adaptées.
...
Avec une approche classique de la mise en forme, le fichier CSS aurait une référence distincte au fichier image individuel sur le serveur pour chaque état de l’élément d’interface utilisateur. Voici un exemple de page CSS pour la mise en forme d’un bouton de défilement :

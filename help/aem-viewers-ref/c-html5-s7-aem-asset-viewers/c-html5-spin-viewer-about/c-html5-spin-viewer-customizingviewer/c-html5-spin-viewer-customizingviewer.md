...
description: Toutes les personnalisations visuelles et la plupart des personnalisations de comportement pour la visionneuse à 360° sont effectuées par la création d’une page CSS personnalisée.
keywords: responsive
...
Toutes les personnalisations visuelles et la plupart des personnalisations de comportement pour la visionneuse à 360° sont effectuées par la création d’une page CSS personnalisée.
...
La visionneuse est fournie avec des fichiers CSS prêts à l’emploi, pour les schémas de couleurs &quot;clair&quot; et &quot;foncé&quot;. La version &quot;light&quot; est utilisée par défaut, mais vous pouvez passer à la version &quot;dark&quot; à l’aide de la feuille CSS standard suivante :
...
Lors de la création d’une page CSS personnalisée, gardez à l’esprit que la visionneuse affecte `.s7spinviewer` à son élément DOM de conteneur. Si vous utilisez un fichier CSS externe transmis avec `style=` commande, utiliser `.s7spinviewer` classe en tant que classe parente dans le sélecteur descendant pour vos règles CSS. Si vous effectuez des styles intégrés sur la page web, qualifiez également ce sélecteur avec un identifiant de l’élément DOM du conteneur comme suit :
...
Pour vous aider à créer une page CSS réactive, la visionneuse prend en charge les marqueurs CSS qui sont des classes CSS spéciales dynamiquement affectées à l’élément de conteneur de la visionneuse de niveau supérieur. Ces classes sont basées sur la taille de la visionneuse au moment de l’exécution et le type d’entrée utilisé sur l’appareil actuel.
...
L’utilisation de marqueurs CSS est la méthode la plus souple pour créer des CSS réactives. Cela est dû au fait qu’il vous permet de cibler non seulement la taille de l’écran de l’appareil, mais aussi la taille réelle de la visionneuse, ce qui peut s’avérer utile pour les mises en page de conception réactive.
...
Avec une approche classique de la mise en forme, le CSS dispose d’une référence distincte au fichier image individuel sur le serveur pour chaque état de l’élément d’interface utilisateur. Voici un exemple de page CSS pour la mise en forme du bouton de zoom avant :
...
Voici une documentation de référence sur les éléments de l’interface utilisateur qui s’applique à la visionneuse de supports variés :

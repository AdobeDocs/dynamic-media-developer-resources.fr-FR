...
Lors de la création d’un CSS personnalisé, gardez à l’esprit que la visionneuse attribue `.s7flyoutviewer` classe à son élément DOM conteneur. Si vous utilisez un fichier CSS externe transmis avec `style=` commande, utilisez `.s7flyoutviewer` classe comme classe parente dans le sélecteur descendant pour vos règles CSS. Si vous effectuez des styles incorporés sur la page web, qualifiez également ce sélecteur avec un identifiant de l’élément DOM du conteneur comme suit :
...
Pour faciliter la création d’une feuille de style CSS réactive, la visionneuse prend en charge les marqueurs CSS qui sont affectés dynamiquement à des classes CSS spéciales à l’élément de conteneur de visionneuse de niveau supérieur. L’affectation est basée sur la taille de la visionneuse au moment de l’exécution et le type d’entrée utilisé sur l’appareil actuel.
...
L’utilisation de marqueurs CSS est le moyen le plus flexible de créer un CSS réactif conçu. En effet, il vous permet de cibler non seulement la taille de l’écran de l’appareil, mais également la taille réelle de la visionneuse, ce qui peut s’avérer utile pour les mises en page web en responsive design.
...
Avec une approche classique du style, le CSS dispose d’une référence distincte au fichier image individuel sur le serveur pour chaque état de l’élément d’interface utilisateur. Voici un exemple de CSS pour le style du bouton de défilement :
...
* [Zoom sur la fenêtre déroulante](r-html5-flyout-viewer-20-customize-flyoutzoomview.md)
* [Mettre en surbrillance](r-html5-flyout-viewer-20-customize-focushighlight.md)
* [Zone de visionneuse principale](r-html5-flyout-viewer-20-customize-mainviewerarea.md)
* [Nuanciers](r-html5-flyout-viewer-20-customize-swatches.md)

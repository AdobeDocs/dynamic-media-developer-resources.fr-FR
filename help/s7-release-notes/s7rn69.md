---
description: '"Notes de mise à jour les plus récentes, décrivant les fonctionnalités, améliorations et correctifs les plus récents d’Adobe Scene7 Publishing System 6.9, composant de la solution Adobe Experience Manager dans Adobe Marketing Cloud."'
...
Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour Adobe Scene7 Publishing System 6.9, composant de la solution Adobe Experience Manager dans Adobe Marketing Cloud.

## Notes de mise à jour de Scene7 Publishing System 6.9 (octobre 2014) {#topic-1d3011ecca71488abe0d8156a9fc1b97}

Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour la version [!DNL Adobe Scene7 Publishing System] 6.9 de la solution Adobe Experience Manager dans la version [!DNL Adobe Marketing Cloud].
...
* ** Visionneuse de catalogue électronique avancée** - Avec une table des matières, une animation de page, des recouvrements de zones cliquables, etc., vous pouvez désormais offrir une expérience d’affichage de catalogue en ligne plus attrayante, et mettre en évidence les produits et services afin que les clients puissent se connecter directement aux pages qui génèrent une augmentation de la conversion.
...
* **Aperçu sécurisé pour Visual Configurator**  : prévisualisez et testez en toute sécurité le rendu visuel des expériences personnalisées avant de les envoyer en production.
* **Fonction SSO (authentification unique)**  : vous pouvez désormais lancer Scene7 à partir de Adobe Marketing Cloud et utiliser votre Adobe ID pour vous connecter à Scene7.

   Si vous souhaitez configurer l’authentification unique afin de pouvoir l’utiliser avec Scene7, contactez le support technique de l’Adobe. Préparez les informations suivantes afin que vos comptes Marketing Cloud et Scene7 puissent être liés : le nom de votre organisation Adobe Marketing Cloud ; le nom de votre société Scene7 ; et votre région géographique.
...
* Surveillance de la fonctionnalité Invalider le cache CDN : après avoir sélectionné votre modèle d’invalidation CDN (voir [Paramètres généraux](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings)) et qu’une requête **[!UICONTROL Invalider le CDN]** est effectuée à partir du menu Fichier dans SPS, un indicateur s’affiche désormais dans l’interface utilisateur qui vous donne une estimation du temps nécessaire pour effacer le cache.

## Visionneuses {#section-fc0577722d0b431dae3506c8d4e5366f}

**Nouvelles fonctionnalités, améliorations et correctifs pour le SDK de la visionneuse HTML5 Scene7 2.8.1**

Généraux

* Les composants qui utilisent l’API `insertBefore()` utilisent désormais l’API `appendChild()`.
...
* Fixe - IE9 : s7sdk.event: PageMouseEvent : cliquer et double-cliquer sur les événements provoque le blocage de la page du navigateur.
* Ajout du composant `TableOfContents` .

FlyoutzoomView

* Fixe - Firefox : Les icônes d’image endommagées s’affichent brièvement sur la fenêtre déroulante jusqu’au chargement des mosaïques.
...
* Ajout de la prise en charge du renvoi d’objets `MediaSetDesc` spécifiques à l’orientation.
* Corrigé : l’affichage d’Imagex échoue si le modificateur IS avec virgule est ajouté à l’ID de ressource.
* Fixe - Imagesets : Les modificateurs IS ajoutés à assetID sont ignorés.
...
Une fois que vous avez configuré votre ordinateur pour accéder au serveur d’évaluation, vous pouvez consulter votre site web pour tester la mise à niveau. Pour les clients qui utilisent des visionneuses HTML5 prêtes à l’emploi, il est recommandé de les tester par rapport à notre serveur d’évaluation standard et s7is1-preview-staging.scene7.com.
...
* Ajout de la prise en charge du défilement de page natif dans les échantillons de visionneuse HTML5
...
* Ajout de la prise en charge de l’affichage des images étroites à l’aide du modificateur de mode de surlignage dans la visionneuse Fenêtre déroulante HTML5
...
* Visionneuse de supports variés HTML5 : Les états du bouton Zoom ne sont pas réinitialisés lors de la permutation des types de ressources
* Visionneuse de supports variés HTML5 : La visionneuse se réduisait en mode réactif si la première ressource était en 2dspinset.
...
* L’image d’affiche peut rencontrer une erreur &quot;taille maximale&quot;. Par conséquent, vous devrez peut-être augmenter le paramètre de limite pour la publication sur hébergeur d’images.
...
## Correctifs de bogues pour Scene7 Publishing System 6.9 {#section-86dc0e8eb85241dbb449221becb12888}
...
* Impossible d’afficher un aperçu des paramètres prédéfinis de la visionneuse vidéo HTML5 prêts à l’emploi.
...
* Le serveur de métadonnées et la recherche par analogie seront abandonnés en 2015 en raison d’une faible adoption. Lorsque la date cible officielle d’obsolescence est annoncée, les clients concernés sont avertis par le support technique d’Adobe.

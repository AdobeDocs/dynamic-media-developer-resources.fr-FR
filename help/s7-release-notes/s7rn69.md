---
description: Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour Adobe Scene7 Publishing System 6.9, composant de la solution Adobe Experience Manager de l’Adobe Marketing Cloud.
seo-description: Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour Adobe Scene7 Publishing System 6.9, composant de la solution Adobe Experience Manager de l’Adobe Marketing Cloud.
...
Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour Adobe Scene7 Publishing System 6.9, composant de la solution Adobe Experience Manager de l’Adobe Marketing Cloud.

## Notes de mise à jour de Scene7 Publishing System 6.9 (octobre 2014) {#topic-1d3011ecca71488abe0d8156a9fc1b97}

Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs de la version [!DNL Adobe Scene7 Publishing System] 6.9 de la solution Adobe Experience Manager de la [!DNL Adobe Marketing Cloud]version.
...
* **Capacité** d’authentification unique (authentification unique) : vous pouvez désormais lancer Scene7 en toute transparence à partir d’un Adobe Marketing Cloud et utiliser votre Adobe ID pour vous connecter à Scene7.

   Si vous souhaitez configurer l’authentification unique pour l’utiliser avec Scene7, contactez l’assistance technique Adobe. Ayez les informations suivantes à portée de main afin que vos comptes Marketing Cloud et Scene7 puissent être liés : le nom de votre organisation d&#39;Adobes Marketing Cloud ; votre nom de société Scene7 ; et votre région géographique.
...
* Le sous-titrage vidéo est désormais disponible dans les `Universal_HTML5_MixedMedia_dark` `Universal_HTML5_MixedMedia_light` visionneuses et les lecteurs prêts à l’emploi. Reportez-vous à [Ajout de sous-titres aux vidéos](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/video/adding-captions-video.html).

## Impression en ligne {#section-17c5591d279a48d3b0aea085a19d6321}

* **Adobe Illustrator Plug-in for Web-to-Print and Adobe InDesign Plug-in for Web-to-Print** - These two new plug-in replace the previous CSXS (Creative Suite Extension) plug-in for Web-to-Print in Scene7. (voir [Démarrage rapide : Publication de modèles](http://help.adobe.com/en_US/scene7/using/WS447286D6-49D9-427c-906A-3A8169974681.html)).

## Plate-forme {#section-6f0721d6ab10495a927f509f026d70f4}

* Requêtes Secure Testing for Render Server - Secure Testing pour les fichiers de diffusion d’images de base et les fichiers vidéo est désormais étendu afin d’inclure également des vignettes. See [Testing assets before making them public](http://help.adobe.com/en_US/scene7/using/WSd968ca97bf00cf72-5eeee3a113268dc80f5-8000.html).
* Surveillance de la fonctionnalité de cache Invalider sur le réseau de diffusion de contenu : après avoir sélectionné votre modèle d’invalidation sur le réseau de diffusion de contenu (voir Paramètres [](http://help.adobe.com/en_US/scene7/using/WSCCBA9D3A-06A3-4f29-AF6B-36CBB2A655F1.html)généraux) et qu’une demande d’invalidation sur le réseau de diffusion de contenu **** est effectuée à partir du menu Fichier dans SPS, un indicateur apparaît désormais dans l’interface utilisateur pour vous donner une estimation du temps nécessaire pour effacer le cache.
...
* Prise en charge Ajoutée des `setCSS()`méthodes, `setModifier()`et `dispose()` méthodes.
* Prise en charge Ajoutée des méthodes `getWidth()`et `getHeight()` méthodes.
...
* Composant Ajouté `TableOfContents` .
...
* Prise en charge Ajoutée pour le renvoi d’ `MediaSetDesc` objets spécifiques à l’orientation.
...
* Corrigé : l’image est déplacée à droite si elle `DIV` possède `text-align:center` un style.
...
**Correction de bogues pour Scene7 OnDemand 5.0.2**
...
Voir aussi le Guide [de référence des visionneuses](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/home.html)Scene7.

* Mise à jour de la prise en charge d’Internet Explorer 11
* Modification des visionneuses pour utiliser les requêtes de suivi plutôt que les requêtes de page pour le suivi Adobe
...
* Prise en charge Ajoutée de l’affichage d’images étroites à l’aide du modificateur de mode de surlignage dans la visionneuse Fenêtre déroulante HTML5
...
* Les légendes vidéo nécessitent un ensemble de règles de société si elles hébergent une page HTML diffusée à partir d’un serveur externe qui n’est pas un serveur Scene7. Contactez l&#39;assistance technique Adobe pour obtenir de l&#39;aide.

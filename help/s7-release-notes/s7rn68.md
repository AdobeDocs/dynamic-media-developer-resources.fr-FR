---
description: Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour Adobe Scene7 Publishing System 6.8, composant de la solution Adobe Experience Manager dans Adobe Marketing Cloud.
seo-description: Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour Adobe Scene7 Publishing System 6.8, composant de la solution Adobe Experience Manager dans Adobe Marketing Cloud.
...
Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs de la version [!DNL Adobe Scene7 Publishing System] 6.8 de la solution Adobe Experience Manager dans le [!DNL Adobe Marketing Cloud]site.
...
* [Nouvelles fonctionnalités et corrections de bogues dans Scene7 SDK 2.7.1](s7rn68.md#section-b82b4f546ce44a769e94fb1493511035)
...
* Ajouté `SetIndicator` à la visionneuse de zoom HTML5 et à la visionneuse de supports variés HTML5 pour représenter visuellement les échantillons sur les périphériques mobiles.
* Ajoutée `getComponent` API pour autoriser l’accès externe aux composants de la visionneuse HTML5.
...
* Ajouté le bouton &quot;Sélectionner tout&quot; pour faciliter l’obtention du lien et du code incorporé pour le partage sur les réseaux sociaux.
...
* Ajouté `unload()` l’API à SpinView et ZoomView pour permettre la disposition du composant au moment de l’exécution.
...
* Affichage de zoom pour masquer `IconEffect` un composant lorsque le zoom n’est pas possible
...
* Convertir le code HTML du courrier électronique de mot de passe oublié en texte brut pour empêcher la création de blocs.
* Suppression du rôle par défaut.
* Outre la possibilité de télécharger du contenu vers Scene7 Publishing System au moyen du téléchargement d’annuaires de serveur (ftp) et des URL de téléchargement, vous pouvez désormais télécharger du contenu à l’aide de HTTP POST. Voir l’API [](/help/aem-ips-api/c-overview.md) Image Production System pour plus d’informations sur l’utilisation de HTTP POST.
...
* La visionneuse vidéo HTML5 empêche l’accès au module externe Flash de se débloquer.
...
* Message d’alerte Ajouté lorsque vous modifiez la configuration de Adobe Analytics, puis tentez d’échapper à la configuration sans l’enregistrer.
...
* Lors de l’initialisation des paramètres de société, Scene7 Publishing System envoie l’option duplicata `<defaultValue>` dans l’API `<createMetadataField>`.
...
* Lors de la suppression de la valeur d’un champ défini par l’utilisateur Date/Heure, Scene7 Publishing System n’envoie aucune requête à IPS (Image Publishing System).
...
* L’URL du courrier électronique Mot de passe oublié est HTTP, même si elle provient de SSL Scene7 Publishing System.
...
* Les informations du compte FTP Scene7 sont manquantes dans le message de bienvenue pour un compte d’évaluation.
...
* La Société d’étiquettes ne s’affiche pas complètement dans la page de configuration d’Adobe Analytics.
* Impossible d&#39;importer les métadonnées.

## Problèmes connus {#section-b2608177928b4174b080d89c938bb0d3}

* La mise à jour de mai 2014 d’Adobe Illustrator CC (Creative Cloud) et d’Adobe InDesign CC devrait entraîner des incompatibilités avec les modules externes qui créent le fichier FXG Scene7. Ces incompatibilités affectent les utilisateurs de la solution d’impression en ligne de Scene7. Si vous utilisez Scene7 Web-to-Print, ne mettez pas à niveau Illustrator et InDesign vers cette mise à jour Creative Cloud. Continuez plutôt à utiliser la version antérieure à mai 2014. Adobe Systems prévoit de rétablir la compatibilité de la création FXG de Scene7 avec les versions plus récentes d’Illustrator et d’InDesign au cours de la période de l’automne 2014.
...
* Scene7 Creative Suite Extensions (panneaux Flash) sera abandonné à la fin de 2014. Le développement des workflows d’assimilation de contenu se concentre sur Adobe Experience Manager Dynamic Media et Creative Cloud. Pour Scene7, les fonctions de téléchargement et d’exportation dans Scene7 Publishing System seront conservées et peuvent être utilisées à la place.
* Depuis mai 2014, Akamai ne prend plus en charge les notifications par courrier électronique lorsque l’invalidation du cache dans Scene7 est terminée. Le temps de traitement normal pour l’invalidation du CDN est d’environ huit minutes. S’il existe une file d’attente importante de requêtes ou un grand nombre d’URL, le temps de traitement peut prendre un peu plus de temps.
...
## Nouvelles fonctionnalités et corrections de bogues dans Scene7 SDK 2.7.1 {#section-b82b4f546ce44a769e94fb1493511035}
...
* Fixe : `selectSwatch` ne se déclenche pas lorsqu’elle est incorporée dans la `initViewer()` fonction.
...
* Fixe : L’icône Partage sur les réseaux sociaux ne disparaît pas avec l’icône `controlbar` Zoom.
...
* Prise en charge Ajoutée du `initialBitRate` modificateur pour sélectionner le débit binaire utilisé pour la lecture initiale sur le bureau.
...
* Fixe : Protocole HTTPS générant un avertissement de sécurité sur IE8 à l’aide de la lecture Flash.
* Fixe : La taille de la vidéo n’est pas correcte en mode plein écran dans Safari 5 avec Flash.
...
## Nouvelles fonctionnalités et corrections de bogues dans S7OnDemand 5.0.1 {#section-ba3a595134144711bf27dd8dca249e5d}

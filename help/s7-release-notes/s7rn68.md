...
Notes de mise à jour les plus récentes, décrivant les nouvelles fonctionnalités, les améliorations et les correctifs pour [!DNL Adobe Scene7 Publishing System] 6.8, partie de la solution Adobe Experience Manager dans le [!DNL Adobe Experience Cloud].
...
* Ajout de la prise en charge de la conception web réactive : diffusion d’images réactive basée sur les points d’arrêt et les visionneuses HTML5 réactives (jeux d’images, vidéo, supports variés, catalogue électronique, visionneuses à 360°, visionneuses d’échantillons, zoom) qui redimensionnent dynamiquement en fonction de la mise en page.
...
* Visionneuses HTML5 permettant d’utiliser des marqueurs d’entrée et de taille CSS pour contrôler l’affichage sur les ordinateurs de bureau et les appareils mobiles.
...
* Ajout `SetIndicator` à la visionneuse de zoom HTML5 et à la visionneuse de supports variés HTML5 pour représenter visuellement les échantillons sur les appareils mobiles.
* Ajout `getComponent` API permettant un accès externe aux composants de la visionneuse HTML5.
...
* `stagesize` à la visionneuse Fenêtre déroulante HTML5.
* `initComplete` gestionnaire vers les visionneuses HTML5 pour déclencher des fonctions après les visionneuses.
...
* ZoomView à masquer `IconEffect` composant lorsque le zoom n’est pas possible
...
* Pour éviter tout liste bloquée, convertissez le HTML de formulaire de courrier électronique avec mot de passe oublié en texte brut.
* Suppression du rôle par défaut.
* Outre la possibilité de charger du contenu vers Scene7 Publishing System au moyen du téléchargement de répertoire de serveur (ftp) et des URL de chargement, vous pouvez désormais transférer du contenu à l’aide du POST HTTP. Voir [API du système de production d’images](/help/aem-ips-api/c-overview.md) pour plus d’informations sur l’utilisation du POST HTTP.
...
* Le paramètre `contenturl` est issue de l’URL de prévisualisation de la visionneuse HTML5 du serveur de publication.
...
* &quot;Space After&quot; ne fonctionne pas; doit utiliser `sa`, pour l’instant, `margb`.
...
* Corrigé : Les coordonnées du conteneur sont incorrectes si le conteneur est déplacé avec une transformation CSS.
...
* Corrigé : `selectSwatch` ne se déclenche pas lorsqu’elle est incorporée dans la méthode `initViewer()` fonction .
...
* Corrigé : Icône Partage sur les réseaux sociaux ne disparaît pas avec `controlbar` sur Zoom.
...
* Ajout de la prise en charge de `initialBitRate` pour sélectionner le débit binaire utilisé pour la lecture initiale sur le bureau.
...
* Corrigé : Il n’est pas possible de sélectionner des échantillons dans la visionneuse de médias après la lecture de vidéos sur iPhone.

---
description: Restaure les fichiers de la corbeille.
seo-description: Restaure les fichiers de la corbeille.
...
Restaure les fichiers de la corbeille.
...
|---|---|---|---|
| ` *`companyHandle`*` | `xsd:string` | Oui | La poignée d’un avec les ressources à restaurer. |
...
| ` *`warningCount`*` | `xsd:int` | Oui | Nombre d’avertissements générés lorsque l’opération tentait de restaurer des fichiers à partir de la corbeille. |
...
| ` *`errorDetailArray`*` | `types:AssetOperationFaultArray` | Non | Tableau des détails associés aux fichiers qui ont généré des erreurs lorsque l’opération tentait de restaurer des fichiers à partir de la corbeille. |

...
|---|---|---|---|
| ` *`Expression de code`*` | `xsd:string` | Oui | La poignée du. |
| ` *`Expression de code`*` | `xsd:string` | Non | Moteur utilisé dans la génération. Voir Styles de police. |
| ` *`Expression de code`*` | `xsd:string` | Non | Le nom d’utilisateur de la ressource à  pour les ressources générées. |
| ` *`Expression de code`*` | `xsd:string` | Non | Poignée de la ressource à  pour les ressources et les moteurs utilisés dans sa génération. |
...
Cet exemple de code renvoie des informations sur les ressources générées à partir d’une ressource spécifique. Il ne récupère pas les informations sur les étapes utilisées pour générer la ressource spécifiée. La réponse est tronquée pour plus de brièveté.
